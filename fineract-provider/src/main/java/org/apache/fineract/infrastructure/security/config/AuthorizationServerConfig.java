...
public class AuthorizationServerConfig {
    ...
    public SecurityFilterChain publicEndpoints(HttpSecurity http) throws Exception {
        ...
        http.securityMatcher("/swagger-ui/**", "/fineract.json", "/actuator/**", "/legacy-docs/apiLive.htm", "/api/v1/public/**", "/api-docs/**", "/v3/api-docs/**")
    ...
    public SecurityFilterChain protectedEndpoints(HttpSecurity http) throws Exception {
        http
                ...
                .oauth2ResourceServer(
                        resourceServer -> resourceServer.jwt(jwt -> jwt.decoder(jwtDecoder()).jwtAuthenticationConverter(authenticationConverter())))
    ...
    public JwtDecoder jwtDecoder() {
        // Sử dụng JwkSetUri để validate JWT tokens từ Keycloak
        // Issuer is http://localhost:9000/realms/fineract (external), but JWKS must be reachable from container
        return NimbusJwtDecoder.withJwkSetUri("http://keycloak:8080/realms/fineract/protocol/openid-connect/certs")
